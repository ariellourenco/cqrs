...
public sealed class SeatsAvailability : Entity, IAggregateRoot
{
    ...
    public void AddSeats(Guid seatType, int quantity) =>
        AddEvent(new AvailableSeatsChanged([new SeatQuantity(seatType, quantity)]));
    ...
    public void CancelReservation(Guid reservationId)
    {
        if (_pendingReservations.TryGetValue(reservationId, out var reservation))
        {
            AddEvent(new SeatsReservationCancelled(
                id: reservationId,
                availableSeatsChanged: [.. reservation.Select(x => new SeatQuantity(x.SeatType, x.Quantity))]));
        }
    }
    ...
    private sealed class Reservation
    {
        ...
        /// If the number of wanted seats is greater than available we reserve all available seats.
        /// </summary>
        public int Actual => Math.Min(Wanted, Math.Max(Remaining, 0) + Pending);

        /// <summary>
        /// Gets the difference of seats availability since last reservation.
        /// </summary>
        public int Difference => Actual - Pending;
    }
}
